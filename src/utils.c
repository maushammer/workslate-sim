...
// use mread instead of mem[]
void hd2(FILE *out, int start, int len)
{
        int y;
        int skip = (start & 0x0F);
        int skip1 = skip;
        ...
        while (len > 0) {
                ...
                for (x = 0; x != 16; ++x) {
                        if (skip || len <= 0) {
                                ...
                                fprintf(out, " %2.2X", mread(start +y + x));
                        }
                        if (x == 7)
                                fprintf(out, " ");
                        --len;
                ...
                fprintf(out, " ");
                for (x = 0; x != 16; ++x) {
                        unsigned char c = mread(start + y + x);
                        if (c < 32 || c > 126) c = '.';
                        if (skip1 || len1 <= 0) {
                                --skip1;
                                fprintf(out, " ");
...
void sim_termios()
{
        ...
        if (!tcgetattr(fileno(stdin), &attr)) {
                ...
                attr.c_iflag &= ~(IXON);  // allow user to type ctrl-s and ctrl-q
                attr.c_lflag&=~ICANON;
